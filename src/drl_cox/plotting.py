...
def plot_survival_curves(
    ...
    for i in range(n_groups):
        ...
        km_time, km_surv, km_ci_lower, km_ci_upper = _kaplan_meier(
            y_group, zeta_group
        )
    ...
    if n_groups == 2:
        ...
        ax.text(
            0.98, 0.02, f"Log-rank p = {p_value:.3e}",
...
def plot_coefficient_path(
    ...
    for i in range(n_features):
        ...
        if i in top_features:
            ...
            y_offset = 0.02 * (np.max(betas) - np.min(betas))
    ...
    ax.set_title(
        title or "Coefficient Path: Impact of Robustness Parameter",
        fontweight="bold",
        pad=20
    )
    ...
    n_nonzero = [np.sum(np.abs(betas[i, :]) > 1e-6) for i in range(len(epsilons))]
...
def plot_cv_results(
    ...
    # Aggregate results
    cv_summary = cv_dataframe.groupby("epsilon")["score"].agg(
        ["mean", "std", "count"]
    ).reset_index()
    ...
    # Mark best epsilon
    if mark_best:
        best_idx = cv_summary["mean"].idxmax()
        best_eps = cv_summary.loc[best_idx, "epsilon"]
        best_score = cv_summary.loc[best_idx, "mean"]
    ...
    ax.set_title(
        title or f"Cross-Validation Results: {metric_name} vs Robustness",
        fontweight="bold",
        pad=20
    )
    ax.legend(loc="best", framealpha=0.95)
...
def plot_concordance_over_time(
    ...
    # Plot each model
    for (model_name, beta), color in zip(models_dict.items(), colors):
        concordances = []
    ...
    ax.set_title(
        title or "Time-Dependent Model Performance",
        fontweight="bold",
        pad=20
    )
    ax.legend(loc="best", framealpha=0.95)
...
def _add_risk_table(
    ...
    # Add table headers
    ax.text(
        -0.1, table_y_start + 0.05, "At Risk:",
        transform=ax.transAxes,
        fontsize=8,
        fontweight="bold",
    )
    ...
    # Add risk numbers
    for i, (row, y_pos) in enumerate(zip(table_text, y_positions)):
        # Group label
        ax.text(
            -0.05, y_pos, row[0],
            transform=ax.transAxes,
        ...
        # Numbers at risk
        for x, val in zip(x_positions, row[1:]):
            ax.text(
                x, y_pos, val,
                transform=ax.transAxes,
                fontsize=8,
...
def create_tutorial_plots():
    ...
    from .drl_cox import fit_drl_cox, cross_validate_epsilon
