...
class SurvivalStandardScaler:
    ...
    def __post_init__(self) -> None:
        ...
        self._ipcw_weights: np.ndarray | None = None
        ...
        if self.robust:
            ...
            if self.with_mean:
                if sample_weight is not None:
                    # Weighted median
                    self.mean_ = np.array(
                        [self._weighted_median(X[:, j], sample_weight) for j in range(n_features)]
                    )
                else:
            ...
            if self.with_std:
                ...
                scale = np.asarray(median_abs_deviation(X, axis=0, scale="normal"), dtype=float)
            ...
            if self.with_std:
                if sample_weight is not None:
                    # Weighted variance
                    var = np.average((X - self.mean_) ** 2, axis=0, weights=sample_weight)
                    scale = np.sqrt(var)
                else:
    ...
    def _compute_ipcw_weights(
        ...
        n = len(y)
        weights = np.ones(n)
        g_floor = 1.0 / n  # keep weights finite when the censoring survival hits zero
        ...
        for t in unique_times:
            ...
            if at_risk > 0:
                ...
                mask = y == t
                weights[mask & (zeta == 1)] = 1.0 / max(G_t_prev, g_floor)
                weights[mask & (zeta == 0)] = 1.0 / max(G_t, g_floor)
    ...
    def _weighted_median(
        ...
        return float(sorted_x[cumsum >= cutoff][0])
...
def detect_outliers(
    ...
    if method == "iqr":
        ...
        if return_scores:
            ...
            scores = np.maximum(
                np.max((lower_bound - X) / IQR, axis=1), np.max((X - upper_bound) / IQR, axis=1)
            )
            scores[scores < 0] = 0
        ...
        raise ValueError(f"Unknown method: {method}. Choose from: 'iqr', 'mad', 'isolation', 'lof'")
...
def handle_missing_data(
    ...
    n_neighbors: int = 5,
    max_iter: int = 10,
) -> tuple[SurvivalDataset, np.ndarray | None]:
    """
    ...
    >>> data = SurvivalDataset(
    ...     X=X, y=np.random.exponential(2, 100), zeta=np.random.binomial(1, 0.7, 100)
    ... )
    >>>
    >>> # Impute missing values
    >>> data_clean, mask = handle_missing_data(data, strategy="knn", missing_indicator=True)
    """
    X = data.X.copy()
    y = data.y.copy()
    zeta = data.zeta.copy()
    ...
    if n_missing_X == 0 and n_missing_y == 0 and n_missing_zeta == 0:
        warnings.warn("No missing values found in the data.", stacklevel=2)
        if missing_indicator:
            return data, np.zeros_like(X_missing, dtype=bool)
        return data, None

    # Store missing indicators before imputation
    missing_mask_original = X_missing.copy()
    ...
    if missing_indicator:
        if strategy == "drop":
            ...
            missing_mask_original = missing_mask_original[~np.any(X_missing, axis=1)]
...
def train_test_split_survival(
    ...
    ...     data, test_size=0.3, stratify_by="event", random_state=42
    ... )
    >>>
    ...
    if n_train + n_test > n_samples:
        raise ValueError(
            f"train_size + test_size = {n_train + n_test} is larger than data size = {n_samples}"
        )
    ...
    if stratify_by is None:
        ...
        # Proportional split
        n_event_test = min(len(event_idx), int(round(len(event_idx) * test_frac)))
        n_censor_test = min(len(censor_idx), n_test - n_event_test)

        if shuffle:
            rng.shuffle(event_idx)
            rng.shuffle(censor_idx)

        test_idx = np.concatenate([event_idx[:n_event_test], censor_idx[:n_censor_test]])
        train_idx = np.concatenate([event_idx[n_event_test:], censor_idx[n_censor_test:]])
        ...
        for group in np.unique(time_groups):
            ...
            n_group_test = int(len(group_idx) * test_frac)
        ...
        for stratum in np.unique(strata):
            ...
            n_stratum_test = max(1, int(len(stratum_idx) * test_frac))

            if shuffle:
                rng.shuffle(stratum_idx)

            test_parts.append(stratum_idx[:n_stratum_test])
            train_parts.append(stratum_idx[n_stratum_test:])
    ...
    train_data = SurvivalDataset(
        X=data.X[train_idx], y=data.y[train_idx], zeta=data.zeta[train_idx]
    )

    test_data = SurvivalDataset(X=data.X[test_idx], y=data.y[test_idx], zeta=data.zeta[test_idx])
...
def create_time_dependent_features(
    ...
    >>> data_td = create_time_dependent_features(data, feature_type="polynomial", degree=2)
    ...
    if feature_type == "polynomial":
        ...
        feature_blocks = [y.reshape(-1, 1) ** d for d in range(1, degree + 1)]

        # Normalize time features
        time_features = np.hstack(feature_blocks)
        ...
        for i in range(len(knots) - 1):
            ...
            feature_blocks.append(basis.reshape(-1, 1))
        ...
        feature_blocks = [(y >= t).astype(float).reshape(-1, 1) for t in time_points]
...
def discretize_time(
    ...
    --------
    >>> data = simulate_cox_data(n=200, d=5)
    >>> data_disc, intervals = discretize_time(data, n_intervals=5, strategy="quantile")
...
def balance_censoring(
    ...
    random_state: int | None = None,
) -> SurvivalDataset | tuple[SurvivalDataset, np.ndarray]:
    """
    ...
    >>> data_balanced = balance_censoring(data, target_rate=0.5, method="subsample")
    ...
    if method == "subsample":
        ...
        if current_rate < target_rate:
            ...
            if n_censored_keep < n_censored:
                censor_idx_keep = rng.choice(censor_idx, n_censored_keep, replace=False)
            ...
            if n_events_keep < n_events:
                event_idx_keep = rng.choice(event_idx, n_events_keep, replace=False)
        ...
        return SurvivalDataset(X=data.X[keep_idx], y=data.y[keep_idx], zeta=data.zeta[keep_idx])
...
def validate_survival_data(
    ...
    if check_finite:
        if np.any(~np.isfinite(data.X)):
            ...
            diagnostics["issues"].append(f"Non-finite values in X: {n_nan} NaN, {n_inf} Inf")
            is_valid = False
    ...
    if check_positive_times:
        if np.any(data.y <= 0):
            ...
            diagnostics["issues"].append(f"{n_negative} non-positive survival times")
            is_valid = False
    ...
    if check_event_indicator:
        ...
        if not set(unique_zeta).issubset({0, 1}):
            diagnostics["issues"].append(f"Invalid event indicators: {unique_zeta}")
            is_valid = False
    ...
    if remove_zero_variance:
        ...
        if len(zero_var_features) > 0:
            diagnostics["warnings"].append(f"Zero-variance features: {zero_var_features.tolist()}")
    ...
    diagnostics["feature_ranges"] = [
        (data.X[:, j].min(), data.X[:, j].max()) for j in range(data.X.shape[1])
    ]

    # Check for extreme event rate
    if diagnostics["event_rate"] < 0.1:
        diagnostics["warnings"].append(f"Low event rate: {diagnostics['event_rate']:.1%}")
    elif diagnostics["event_rate"] > 0.9:
        diagnostics["warnings"].append(f"High event rate: {diagnostics['event_rate']:.1%}")
    ...
    if verbose:
        ...
        print(
            f"Time range: [{diagnostics['time_range'][0]:.2f}, {diagnostics['time_range'][1]:.2f}]"
        )
...
def compute_feature_importance(
    ...
    >>> importance = compute_feature_importance(data, result.beta, method="permutation")
    ...
    if max_importance > 0:
        importance_df["relative_importance"] = importance_df["importance"] / max_importance
    else:
