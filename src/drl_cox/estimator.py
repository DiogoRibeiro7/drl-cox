...
class DRLCoxEstimator(BaseEstimator):
    ...
    solver : str, default="ECOS"
        CVXPY solver to use. Options include:
        - "ECOS": Fast and reliable for most problems
        - "SCS": Scalable for large problems
        - "MOSEK": Commercial solver (if installed)
        - "CLARABEL": Open-source alternative
        
    solver_opts : dict or None, default=None
        Solver-specific options passed to CVXPY.
    ...
    def fit(
        ...
        X = self._validate_data(X, dtype=np.float64, ensure_2d=True, ensure_min_samples=2)
    ...
    def _validate_params(self) -> None:
        ...
        if not isinstance(self.epsilon, (int, float)):
            raise TypeError(
                f"epsilon must be a number, got {type(self.epsilon).__name__}"
            )
        if self.epsilon < 0:
        ...
        if not isinstance(self.p, (int, float)):
            raise TypeError(
                f"p must be a number, got {type(self.p).__name__}"
            )
        if self.p < 1:
        ...
        if not isinstance(self.gamma, int):
            raise TypeError(
                f"gamma must be an integer, got {type(self.gamma).__name__}"
            )
        if self.gamma < 1:
        ...
        if not isinstance(self.solver, str):
            raise TypeError(
                f"solver must be a string, got {type(self.solver).__name__}"
            )
    ...
    def _check_array(
        ...
        if not isinstance(X, np.ndarray):
            try:
                ...
                raise TypeError(
                    f"{name} must be array-like, got {type(X).__name__}"
                ) from e
        ...
        if ndim is not None and X.ndim != ndim:
            raise ValueError(
                f"{name} must be {ndim}-dimensional, got {X.ndim}-dimensional array"
            )
...
def make_drl_cox_scorer(
    ...
    **metric_kwargs : dict
    ...
    """
    def scorer(estimator, X, y, zeta=None):
        ...
        if metric == "cindex":
            ...
            score = time_dependent_auc_iAUC(
                risk_scores, y, zeta, **metric_kwargs
            )
        else:
